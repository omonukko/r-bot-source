...
client.on("messageCreate", (message) => {
  try {
    if (message.author.bot || !allowedGuilds.includes(message.guild.id)) {
      return;
    }
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.commandName === "chat-log") {
      ...
      const fileName = `CHATLOG-byR.BOT v2.${fileFormat}`;
...
async function sendPagination(interaction) {
  try {
    const pages = [
      {
        title: `R.BOT v2 help`,
        content: `> P.1 **管理:PREFIXコマンド**\n> P.2 **サーバー関連:PREFIXコマンド**\n> P.3 **その他:PREFIXコマンド**\n> P.4 **ユーザー管理**\n> P.5 **情報表示**\n> P.6 **SETUP**\n> P.7 **サーバー管理**\n> P.8 **ツール**\n> P.9 **その他**\n製作者: <@1178414826184265819>\n\nサポートサーバー\nhttps://discord.gg/rserver\n\n公式サイト\nhttps://ruisana.github.io/rbot-support`,
      },
      {
        title: "管理:PREFIXコマンド",
        content: "ページ1",
      ...
      },
      {
        ...
        fields: [
          ...
          { name: "Make it a quote風画像生成", value: "```reply @R.BOT```" },
          {
            name: "NSFW Neko画像生成",
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.commandName === "invite") {
      const embed = new MessageEmbed()
        .setTitle("招待リンク")
        .setDescription("下記のボタンからR.BOTをサーバーに招待できます。")
        .setColor("RANDOM")
        .setTimestamp();
  ...
  try {
    ...
    if (member && member.presence) {
      ...
      const embed = new MessageEmbed()
        ...
        .addField("Bot", user.bot ? "はい" : "いいえ", true)
...
client.on("interactionCreate", async (interaction) => {
  try {
    ...
    if (interaction.commandName === "warning") {
      const fetchMember = require("./fetchMember");
      const user = interaction.options.getUser("user");
      const reason = interaction.options.getString("reason");
      if (!interaction.member.permissions.has("ADMINISTRATOR")) {
        return interaction.reply({
...
client.on("messageCreate", (message) => {
  try {
    if (message.content.startsWith("r.timeout") && !message.author.bot) {
      if (!message.member.permissions.has("ADMINISTRATOR"))
        return message.reply({
          content: "サーバー管理者しか使えません",
          ephemeral: true,
...
client.on("message", async (message) => {
  try {
    if (message.content.startsWith("r.untimeout") && !message.author.bot) {
      if (!message.member.permissions.has("ADMINISTRATOR"))
        return message.reply({
          content: "サーバー管理者しか使えません",
          ephemeral: true,
